...
<head>
    ...
    <link href="<%=prefixPath %>/css/small_list.css" rel="stylesheet" />
                ...
                $.ajax({
                    ...
                    success:function(result){
                        ...
                        for (i=result.data.length-1; i>0; i--) {
                            ...
                            if(result.data[i].top==1){
                                li.innerHTML ='  <div class="mui-card">\n' +
                                    '                            <div class="mui-card-header mui-card-media">\n' +
                                    '                                <a  href="<%=prefixPath %>/article/article?id='+(result.data[i].id)+'">\n' +
                                    '                                    <div class="title_t">\n' +
                                    '                                            '+(result.data[i].title)+'\n' +
                                    '                                    </div>\n' +
                                    '                                </a>\n' +
                                    ...
                                    '                                <a  href="<%=prefixPath %>/article/article?id='+(result.data[i].id)+'">\n' +
                                    '                                    <img src="${contentPath}'+(result.data[i].titleImg)+'" alt="" width="100%">\n' +
                                    '                                </a>\n' +
                                    '                            </div>\n' +
                                    '                            <div class="mui-card-footer2">\n' +
                                    '                                <a  href="<%=prefixPath %>/article/article?id='+(result.data[i].id)+'">\n' +
                                    '                                    <p class="p_width2"><span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>'+(result.data[i].articleIntro)+'</p>\n' +
                                    '                                </a>\n' +
                                    '                                <div class="right-3">\n' +
                                    '                    <span class="time">\n' +
                                    ...
                                    '                        <span class="mun">'+(result.data[i].readingQuantity)+'</span>\n' +
                                    '                    </span>\n' +
                                    '                                    <span class="dianZhan">\n' +
                                ...
                                li.innerHTML ='  <a  href="<%=prefixPath %>/article/article?id='+(result.data[i].id)+'">\n' +
                                    '                                    <img class=" mui-media-object mui-pull-left xmb_img" src="${contentPath}'+(result.data[i].titleImg)+'">\n' +
                                    '                                    <div class="mui-media-body chaochu">\n' +
                                    '                                        <div style="color: black;font-weight: bold;">\n' +
                                    '                                            '+(result.data[i].title)+'\n' +
                                    '                                        </div>\n' +
                                    '                                        <p class="zt_2">'+(result.data[i].articleIntro)+'</p>\n' +
                                    '                                    </div>\n' +
                                    '                                </a>\n' +
                                    ...
                                    '\n' +
                                    '                                    <span class="liuLan">\n' +
                                    '                                      <img src="<%=prefixPath %>/imgs/ios/17@2x.png" alt="" />'+(result.data[i].readingQuantity)+'\n' +
                                    '                                    </span>\n' +
                                    '                                    <span class="dianZhan">\n' +
                                    '                                        <a href="javascript:void(0)"  onclick=\'thumbUp('+(result.data[i].id)+')\'>\n' +
                                    '                                              <img src="<%=prefixPath %>/imgs/ios/d@2x.png" alt="" />'+(result.data[i].thumbUpQuantity)+'\n' +
                ...
                $.ajax({
                    ...
                    success:function(result){
                        ...
                        for (i=0; i<result.data.length; i++) {
                                li = document.createElement('li');
                                if(i==0){
                                    li.innerHTML ='  <div class="small_dt">\n' +
                                        '                            <div class="mui-card-header mui-card-media">\n' +
                                        '                                <a  href="<%=prefixPath %>/article/article?id='+(result.data[i].id)+'">\n' +
                                        '                                    <div class="title_t">\n' +
                                        '                                            '+(result.data[i].title)+'\n' +
                                    ...
                                    li.innerHTML ='  <a  href="<%=prefixPath %>/article/article?id='+(result.data[i].id)+'">\n' +
                                        '                                    <img class=" mui-media-object mui-pull-left xmb_img" src="${contentPath}'+(result.data[i].titleImg)+'">\n' +
                                        '                                    <div class="mui-media-body chaochu">\n' +
                                        '                                        <div class="small_xt" style="color: black;font-weight: bold;">\n' +
                                        '                                            '+(result.data[i].title)+'\n' +
                                        '                                        </div>\n' +
                                        '                                        <p class="zt_2">'+(result.data[i].articleIntro)+'</p>\n' +
                                        '                                    </div>\n' +
